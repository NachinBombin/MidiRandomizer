...
class VoicesFragment : Fragment(), MidiService.MidiEventListener {
    ...
    private fun buildIndependentPanel(voiceId: Int, onSync: () -> Unit): LinearLayout {
        ...
        rootSpinner.adapter = ArrayAdapter(ctx, android.R.layout.simple_spinner_item,
            listOf("Follow Main","C","C#","D","D#","E","F","F#","G","G#","A","A#","B"))
            .also { it.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item) }
        ...
        // Use enum .name (built-in) — no custom .label extension needed
        ...
        jitterSpinner.adapter = ArrayAdapter(
            ctx, android.R.layout.simple_spinner_item,
            JitterType.entries.map { it.name.lowercase().replaceFirstChar { c -> c.uppercase() } }
        ).also { it.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item) }
        ...
        velSpinner.adapter = ArrayAdapter(
            ctx, android.R.layout.simple_spinner_item,
            VelocityPattern.entries.map { it.name.lowercase().replaceFirstChar { c -> c.uppercase() } }
        ).also { it.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item) }
        ...
        mSpinner.adapter = ArrayAdapter(
            ctx, android.R.layout.simple_spinner_item,
            MelodicLogicStyle.entries.map { it.name.lowercase().replaceFirstChar { c -> c.uppercase() } }
        ).also { it.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item) }
    ...
    private fun readIndependentConfig(panel: LinearLayout): IndependentConfig {
        ...
        return IndependentConfig(
            ...
            proSettings   = ProSettings(
                // jitterAmount is Int (0-100)
                jitterAmount     = seekVal("jitAmt"),
                jitterType       = JitterType.entries[
                    panel.findViewWithTag<Spinner>("jitType")?.selectedItemPosition ?: 0],
                velocityPattern  = VelocityPattern.entries[
                ...
                melodicLogicStyle= MelodicLogicStyle.entries[
    ...
    private fun applyToPanel(panel: LinearLayout, cfg: VoiceConfig) {
        ...
        if (cfg.mode == VoiceMode.HARMONY) {
            ...
            panel.findViewWithTag<SeekBar>("indVel")?.progress    = cfg.independentConfig.velocity
            panel.findViewWithTag<SeekBar>("indMidiCh")?.progress = cfg.independentConfig.midiChannel
            panel.findViewWithTag<Spinner>("indStyle")?.setSelection(cfg.independentConfig.style.ordinal)
            panel.findViewWithTag<Spinner>("indRoot")?.setSelection(cfg.independentConfig.rootNote)
            panel.findViewWithTag<RadioButton>("sharedPro")?.isChecked  = cfg.independentConfig.useSharedPro
            panel.findViewWithTag<RadioButton>("customPro")?.isChecked  = !cfg.independentConfig.useSharedPro
            panel.findViewWithTag<View>("customProPanel")?.visibility   =
                if (cfg.independentConfig.useSharedPro) View.GONE else View.VISIBLE
        }
    }
            ...
            when (val c = group.getChildAt(i)) {
                ...
                is Spinner -> c.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                    override fun onItemSelected(a: AdapterView<*>?, v: View?, p: Int, id: Long) {
                        if (!isUpdatingFromSync) onChange()
                    }
                    override fun onNothingSelected(a: AdapterView<*>?) {}
                }
                is RadioButton -> c.setOnCheckedChangeListener { _, checked ->
                    if (checked && !isUpdatingFromSync) onChange()
                }
