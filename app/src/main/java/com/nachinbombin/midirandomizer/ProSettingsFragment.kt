...
class ProSettingsFragment : Fragment() {
    ...
    private fun populateSpinners() {
        spinnerJitterType.adapter    = makeAdapter(JitterType.entries.map { enumDisplayName(it.name) })
        spinnerVelPattern.adapter    = makeAdapter(VelocityPattern.entries.map { enumDisplayName(it.name) })
        spinnerMelodicEngine.adapter = makeAdapter(MelodicEngine.entries.map { enumDisplayName(it.name) })
        spinnerLogicStyle.adapter    = makeAdapter(MelodicLogicStyle.entries.map { enumDisplayName(it.name) })

        val curvesNames = listOf("Flat", "Rising arch", "Falling arch", "Sawtooth", "S-curve")
        spinnerGesturePitch.adapter    = makeAdapter(curvesNames)
        spinnerGestureRegister.adapter = makeAdapter(curvesNames)
        spinnerGestureDensity.adapter  = makeAdapter(curvesNames)
        spinnerGestureVelocity.adapter = makeAdapter(curvesNames)

        spinnerNrtCycle.adapter  = makeAdapter(listOf("Random PLR walk", "LPPL hexatonic", "PRRP octatonic"))
        spinnerPwgMotif.adapter  = makeAdapter(listOf("Classic", "Jazz", "Blues", "Chromatic"))
        spinnerLSystemAxiom.adapter = makeAdapter(listOf("Axiom A (stepwise)", "Axiom B (arch)", "Axiom C (zigzag)", "Axiom D (waves)"))
    }
    ...
    private fun setupListeners() {
        ...
        spinnerMelodicEngine.onItemSelectedListener = simpleSpinner { pos ->
            ...
            // MARKOV legacy switch kept for backward compat with VoiceEngine
        ...
        seekNarmourProcess.setOnSeekBarChangeListener(simpleSeek { p ->
            val v = p / 100f
            current = current.copy(narmourConfig = current.narmourConfig.copy(processVsReversal = v))
            tvNarmourProcess.text = "Process vs Reversal: $p%"; push()
        })
        seekNarmourReturn.setOnSeekBarChangeListener(simpleSeek { p ->
            val v = p / 100f
            current = current.copy(narmourConfig = current.narmourConfig.copy(returnBias = v))
            tvNarmourReturn.text = "Post-leap return bias: $p%"; push()
        })
        seekNarmourLeap.setOnSeekBarChangeListener(simpleSeek { p ->
            val v = p / 100f
            current = current.copy(narmourConfig = current.narmourConfig.copy(maxLeapPenalty = v))
        ...
        seekNrtP.setOnSeekBarChangeListener(simpleSeek { p ->
            val v = p / 5f
            current = current.copy(nrtMelodicConfig = current.nrtMelodicConfig.copy(pWeight = v))
            tvNrtPWeight.text = "P weight: $v"; push()
        })
        seekNrtL.setOnSeekBarChangeListener(simpleSeek { p ->
            val v = p / 5f
            current = current.copy(nrtMelodicConfig = current.nrtMelodicConfig.copy(lWeight = v))
            tvNrtLWeight.text = "L weight: $v"; push()
        })
        seekNrtR.setOnSeekBarChangeListener(simpleSeek { p ->
            val v = p / 5f
            current = current.copy(nrtMelodicConfig = current.nrtMelodicConfig.copy(rWeight = v))
            tvNrtRWeight.text = "R weight: $v"; push()
        })
        ...
        seekPwgPhrase.setOnSeekBarChangeListener(simpleSeek { p ->
            val v = p + 1
            current = current.copy(pwgConfig = current.pwgConfig.copy(phraseLengthMotifs = v))
            tvPwgPhrase.text = "Motifs per phrase: $v"; push()
        })
        seekPwgDir.setOnSeekBarChangeListener(simpleSeek { p ->
            val v = (p / 10f) - 1f
            current = current.copy(pwgConfig = current.pwgConfig.copy(directionBias = v))
            tvPwgDirection.text = directionLabel(v); push()
        })
        ...
        seekLSystemIter.setOnSeekBarChangeListener(simpleSeek { p ->
            val v = p + 1
            current = current.copy(lSystemConfig = current.lSystemConfig.copy(iterations = v))
            tvLSystemIter.text = "Iterations: $v"; push()
        })
        seekLSystemVariance.setOnSeekBarChangeListener(simpleSeek { p ->
            val v = p / 100f
            current = current.copy(lSystemConfig = current.lSystemConfig.copy(ruleVariance = v))
        ...
        seekCaMutation.setOnSeekBarChangeListener(simpleSeek { p ->
            val v = p / 100f
            current = current.copy(cellAutomataConfig = current.cellAutomataConfig.copy(mutationRate = v))
    ...
    private fun directionLabel(v: Float): String = when {
        v < -0.3f -> "Direction bias: descending"
        v > 0.3f  -> "Direction bias: ascending"
        else      -> "Direction bias: neutral"
    }
